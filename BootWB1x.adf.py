...
class OFSDisk:
    ...
    def add_file(self, dir, file, mdays=None, prot=None):
        ...
        if mdays is None:
            mdays = DateStamp.from_gmtime(os.path.getmtime(file_path))
        if prot is None:
            prot = PROT_ARWD
...
class BootWB1x:
    ...
    _LIST = (
        (_DRAWER, 'S', (
            (_OPTBIN, 'S/BootWB1x/Startup-Sequence', PROT_SARWD),
            )),
...
def main(argv):
    if (len(argv) > 2) and (argv[1] in ('release')):
        mdays = DateStamp.from_gmtime(secs=calendar.timegm(
            datetime.datetime.strptime(
